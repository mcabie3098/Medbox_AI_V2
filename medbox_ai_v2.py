    ...
    def pushButton_Update(self):
        ...
        if is_internet():
            ...
            if page==200:
                ...
                # UPDATE IS HERE!
                # NOTE: YOU HAVE TO RENAME GITHUB FOLDER EVERY TIME YOU UPDATE eg. Medbox_AIv3
                window = Tk()
                window.eval('tk::PlaceWindow %s center' % window.winfo_toplevel())
                window.withdraw()                
                if messagebox.askyesno('Medbox AI V2', 'Update is now available! \n\nDo you want to update now?', icon='info') == True:
                    # DOWNLOADING
                    window.deiconify()
                    window.destroy()
                    window.quit()
                    ...
                    #git.Git("").clone("https://github.com/mcabie3098/Medbox_AI_V3.git") # Future purpose Medbox_AIv3 if exist will Update
                    window = Tk()
                    window.eval('tk::PlaceWindow %s center' % window.winfo_toplevel())
                    window.withdraw()
